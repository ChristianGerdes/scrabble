...
module internal Bot =
    ...
    let generateMove (state: State.state) (tiles: Map<uint32, tile>) =
        ...
        let rec findEdge (x, y) direction =
            match direction with
            | Direction.right when isCoordInUse (x, y - 1) || isCoordInUse (x, y - 2) -> findEdge (x, y - 1) direction
            | Direction.down when isCoordInUse (x - 1, y) || isCoordInUse (x - 2, y) -> findEdge (x - 1, y) direction
            | _ -> (x, y)
